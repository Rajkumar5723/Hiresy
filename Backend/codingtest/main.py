...
#     submissions_json = Column(Text, nullable=True)  # [{problem_idx, language, code}]
...
#     submissions: List[dict]   # [{problem_idx, language, code}]
...
# #     # Update application status in main backend
# #     try:
# #         requests.patch(f"http://127.0.0.1:8000/applications/{s.application_id}/status",
# #                        json={"status": "round_3"}, timeout=5)
# #     except Exception as e:
...
#     # ─── Create live HR session and send invitation ─────────────────────
#     try:
...
#             "job_skills": s.job_skills,
#             "github_url": "",          # optional – can be fetched from main backend if needed
#             "github_data": {},
#             "eval_summary": "",        # optional – can be fetched from main backend
#             "scheduled_time": "",      # will show "To be confirmed" in email
#             "hr_email": "",            # empty → HR invite not sent (change if HR email known)
#         }
#         resp = requests.post(livehr_url, json=livehr_data, timeout=10)
#         if resp.status_code == 200:
#             logger.info(f"Live HR session created for application {s.application_id}")
#         else:
#             logger.error(f"Live HR session creation failed: {resp.status_code} {resp.text}")
#     except Exception as e:
#         logger.error(f"Live HR session creation error: {e}")
...
def submit_coding(token: str, req: SubmitCodeRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        livehr_data = {
            ...
            "candidate_email": s.candidate_email,
            "job_title":       s.job_title,
            # Guard against None stored in the DB column
        ...
        if resp.status_code == 200:
            ...
            # Log status code AND full response body so the root cause is visible
            logger.error(
