...
export default function Settings() {
    ...
    useEffect(() => {
        fetch(`http://127.0.0.1:8000/linkedin/status/${hrEmail}`)
            .then(r => r.json())
    ...
    const handleConnect = () => {
        window.location.href = `http://127.0.0.1:8000/linkedin/connect?email=${encodeURIComponent(hrEmail)}`;
    };

    const handleDisconnect = async () => {
        setActionMsg("Disconnecting...");
        await fetch(`http://127.0.0.1:8000/linkedin/disconnect/${hrEmail}`, { method: "DELETE" });
